...
def scan_handler(host: str, port_start: int, port_end: int, report_name="Rapport de scan du ", proto="TCP", parallel_threads=False, socket_timeout=2, verbose=True):
    ...
    def scanner_worker(port_start, port_end):
        for port in range(port_start, port_end+1):
            ...
            if verbose: progressbar(len(logs)-1, total_ports)

    logs.append(f"""Début du scan avec comme paramètres :  \n
...
""")
    ...
    logs.append(timestamp_line(f"Temps total d'éxécution : {datetime.now().replace(microsecond=0) - start_time.replace(microsecond=0)}"))
    report_filename = report_name+datetime.now().strftime('%Y-%m-%d %H:%M')+".log"
